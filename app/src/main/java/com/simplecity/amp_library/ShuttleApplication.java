...
public class ShuttleApplication extends Application {
    ...
    public void onCreate() {
        ...
        if (!client.isAuthenticated()){
            ...
            client.loginAsync("username", "password", Settings.Secure.getString(getApplicationContext().getContentResolver(), Settings.Secure.ANDROID_ID), status -> {
                if (status == Status.SUCCESS) {
                    Log.d(TAG, "Login status: " + status);
                    client.getAllSongsAsync((status1, songs) -> {
